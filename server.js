function PlayerData(id, x, y, speed, mass, name, color) {
    ...
    this.update = function() {
        ...
        if (this.posX >= WORLD_WIDTH - this.radius)
            this.posX = WORLD_WIDTH - this.radius;
        if (this.posX <= this.radius)
            this.posX = this.radius;
        if (this.posY >= WORLD_HEIGHT - this.radius)
            this.posY = WORLD_HEIGHT - this.radius;
...
io.sockets.on('connection', function(socket) {
    ...
    socket.on('signIn', function(data) {
        ...
        PLAYER_LIST[id] = new PlayerData(id, 100, 100, 5, 100, data.name,
                data.color);
        socket.emit('init', {
...
function enemyIsEatable(player, enemy) {
    return distance(player.posX - enemy.posX, player.posY - enemy.posY) < player.radius
...
function eatNearbyFood(player) {
    for ( var i in FOOD_LIST) {
        ...
        if (distance(player.posX - food.posX, player.posY - food.posY) <= player.radius
                - food.radius) {
            player.addMass(5);
            delete FOOD_LIST[i];
