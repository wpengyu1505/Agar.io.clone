...
function Player(posX, posY, speedX, speedY, absSpeed, name, color, mass, radius) {
	...
	this.update = function() {
		...
		if (dist < player.absSpeed) dist = 0;
		...
		//console.log('mouseX=' + mouseX + '|posX=' + this.posX + '|dist=' + dist);
		...
		if (this.posX >= imgBg.width - this.radius) this.posX = imgBg.width - this.radius;
		if (this.posX <= this.radius) this.posX = this.radius;
		if (this.posY >= imgBg.height - this.radius) this.posY = imgBg.height - this.radius;
		if (this.posY <= this.radius) this.posY = this.radius;
		if ((this.posX >= imgBg.width - this.radius) || (this.posX <= this.radius) || (this.posY >= imgBg.height - this.radius) || (this.posY <= this.radius)) {
...
function robotIsEatable(player, robot) {
	return distance(player.posX - robot.posX, player.posY - robot.posY) < player.radius && player.mass > robot.mass
}

function drawBackground() {
	var x = WIDTH - player.posX;
	var y = HEIGHT - player.posY;
	ctx.drawImage(imgBg, WIDTH, HEIGHT, imgBg.width, imgBg.height, x, y, imgBg.width, imgBg.height);
}
 ...
 */
var robotList = {};
var player = new Player(Math.round(Math.random() * WIDTH), Math.round(Math.random() * HEIGHT), 0, 0, 4, "Hadoop", 'green', 50, 10);
//Create robot objects



function createRobot() {
	robotList[robotCount] = new Robot(robotCount, Math.random() * WIDTH, Math.random() * HEIGHT, Math.random() * 10, Math.random() * 10, 6, 'grey', 300, "Spark");
	robotCount += 1;
	console.log("Robot count: " + robotCount);
}

//Robot("Spark", Math.random() * WIDTH, Math.random() * HEIGHT, Math.random() * 10, Math.random() * 10, 6, 'yellow', 500, 10);
//Robot("HBase", Math.random() * WIDTH, Math.random() * HEIGHT, Math.random() * 10, Math.random() * 10, 6, 'red', 750, 10);
